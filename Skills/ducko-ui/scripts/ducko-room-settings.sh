...
on run argv
    ...
    tell application "System Events"
        ...
        tell process "DuckoApp"
            ...
            repeat with elem in allElems
                try
                    if role of elem is "AXMenuItem" and name of elem is "Room Settings…" then
                        click elem
                        return "ok"
                    end if
                end try
            end repeat
            return "ERROR: Room Settings menu item not found (are you an owner/admin?)"
        end tell
    end tell
end run
APPLESCRIPT
...
if [[ "$RESULT" == ok ]]; then
    echo "Room Settings sheet opened for $ROOM_JID"
else
    echo "$RESULT" >&2
    exit 1
fi
