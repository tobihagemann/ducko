...
An isolated conformance **cannot** satisfy a `SendableMetatype` requirement. This surfaces when you pass a metatype (`MyType.self`) to a generic function whose type parameter requires `Sendable`:
