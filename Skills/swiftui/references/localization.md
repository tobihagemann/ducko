...
Guidance for user-facing text: `Text`, `Button`, `Label`, navigation/toolbar titles, alerts, and types that carry localizable strings. For the narrower "verbatim vs localized" decision on a single `Text`, see `text-formatting.md`.
...
Ducko is a multi-package SwiftPM app, so this bundle rule is load-bearing here. Most SwiftUI views live in **`DuckoUI`, a library target** — its resources live in `Bundle.module`, not `Bundle.main`. The existing precedent is `Sources/DuckoUI/Theme/ThemeEngine.swift`, which loads its bundled `Themes` resource via `Bundle.module.url(forResource: "Themes", …)` against the target's `resources: [.copy("Resources/Themes")]`. Only `DuckoApp` (the executable target) is its own main bundle.

Ducko is **not localized yet** — there is no String Catalog, and no `NSLocalizedString` / `String(localized:)` usage in the codebase. So treat this as forward-looking: when localization is added, every user-facing string that lives in `DuckoUI` (or any other library target — `DuckoCore`, `DuckoData`, etc.) must pin the bundle (`Text("…", bundle: #bundle)` / `String(localized: "…", bundle: .module)`), or the lookup silently falls back to the raw key. Strings originating in `DuckoApp` can omit the bundle. If a single pinning helper per target emerges, prefer it over repeating `bundle:` at every call site — but don't introduce one speculatively before there are strings to localize.
